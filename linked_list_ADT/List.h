...
private:
   ...
   int num_elements;
    ...
    unsigned long int countChars() const;
    Node* exchange(Node* &o, Node* n)const;
    void insertNode(Node* before, Node* ins, Node* after);
public:
   ...
   struct Iterator {
	...
	struct Const_Iterator {
		...
		Const_Iterator(pointer ptr) : m_ptr{ptr} {if(ptr == nullptr)std::cout << "oh no" << "\n";}
		...
		// Postfix increment
		Const_Iterator operator++(int) { Const_Iterator tmp = *this; m_ptr = m_ptr->next; return tmp; }

		// Prefix increment
		Const_Iterator& operator--() { m_ptr = m_ptr->prev; return *this; }

		// Postfix increment
		Const_Iterator operator--(int) { Const_Iterator tmp = *this; m_ptr = m_ptr->prev; return tmp; }
   ...
   Iterator begin() { return Iterator(frontDummy->next); }
	Iterator end() { return Iterator(backDummy); }

	Const_Iterator begin() const{ return Const_Iterator(frontDummy->next); }
	Const_Iterator end() const{ return Const_Iterator(backDummy); }
